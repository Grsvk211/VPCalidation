...
def loadConfig():
    try:
        ...
        logging.info(f"Error{ex} line no. {exc_tb.tb_lineno} file name: {exp_fname}")
...
def openTestPlan():
    PT = findInputFiles()[0]
    if len(PT) != 0:
        testPlan = openExcel(getInputFolder() + "\\" + PT)
        return testPlan
    else:
...
def get_last_n_months(month_number, year, n):
    """ Get the last n months relative to a given month. """
    months = []
...
def main(tab):
    ...
    try:
        ...
        # global stop_threads
        # stop_threads = False
        # t1 = threading.Thread(target=excel_popup, args=(tpBook.name,))
        # t1.start()
        # try:
        #     PLM_EE_Book = openExcel(path)
        # except:
        #     stop_threads = True
        # stop_threads = True
        ...
        for i in deliver['cellPositions']:
            ...
            # month = Task_Final_Submission.month
            # Ensure Task_Final_Submission is a datetime object
            # month = 5
            ...
            if isinstance(Task_Final_Submission, datetime):
                ...
                if Task_Final_Submission.month == month_number:
                    ...
                    logging.info("FELP_No,Component,Task_Type,Simple_Complexity,Medium_complexity,New_implementation------->",
                          FELP_No, Component, Task_Type, Simple_Complexity, Medium_complexity, New_implementation)
                    logging.info("SWC_version,Task_received_date,PSA_Review_Deadline,Task_Submission_PSA,Task_Final_Submission,PSA_Final_Submission_Date_requested------->",
                    ...
                    SWC_versions.append(SWC_version), Task_received_dates.append(Task_received_date), PSA_Review_Deadlines.append(PSA_Review_Deadline), Task_Submission_PSAs.append(Task_Submission_PSA), Task_Final_Submissions.append(Task_Final_Submission)
                    ...
                    logging.info("FELP_Nos, Components, Task_Types, Simple_Complexitys, Medium_complexitys, New_implementations------->",
                          FELP_Nos, Components, Task_Types, Simple_Complexitys, Medium_complexitys, New_implementations)
                    logging.info(
                        "SWC_versions, Task_received_dates, PSA_Review_Deadlines, Task_Submission_PSAs, Task_Final_Submissions------->",
                        SWC_versions, Task_received_dates, PSA_Review_Deadlines, Task_Submission_PSAs, Task_Final_Submissions)
                    logging.info("PSA_Final_Submission_Date_requesteds, MCDCs, Qualtiys, misras------->", PSA_Final_Submission_Date_requesteds, MCDCs, Qualtiys, misras)

        hsg = list(zip(FELP_Nos, Components, Task_Types, Simple_Complexitys, Medium_complexitys, New_implementations, SWC_versions, Task_received_dates, PSA_Review_Deadlines, Task_Submission_PSAs, Task_Final_Submissions, PSA_Final_Submission_Date_requesteds, MCDCs, Qualtiys, misras))
        logging.info("hsg--------->", hsg)
        path = getInputFolder() + "\\" + findInputFiles()[1]
        print("path ---->", path)
